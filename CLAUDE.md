...
- `hooks.py` (MkDocs hook, registered in `mkdocs.yml`) adds every entry of `projects/published.yml` to the nav under Projects, Published projects, as an external link, so the menu never needs hand-editing.
