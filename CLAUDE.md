...
- `docs/index.md` puts the full paper list right under the hero (paged, 4 per page) and the newest news rows further down. Both come from `docs/publications/index.md` (marker `all`) and `docs/news/index.md` (marker `latest`) through snippet markers (`<!-- --8<-- [start:all] -->` ... `[end:all]`). Keep new rows inside those markers, and use absolute URLs for internal links inside marked rows, because MkDocs rewrites relative links against the including page.
- `docs/javascripts/paging.js` pages any `<div class="paged" data-size="N">` of `.pub-row`/`.news-row` children and honours `#year` deep links. `docs/javascripts/members.js` adds the search box filter and the per-section counts on the Members page; it runs via `document$` because of instant navigation.
